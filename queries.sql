...
FROM
  animals
WHERE
  date_of_birth BETWEEN 'Jan 1, 2016'
  AND 'Jan 1, 2019';
...
FROM
  animals
WHERE
  date_of_birth BETWEEN 'Jan 1, 1990'
  AND 'Dec 31, 2000'
GROUP BY
  neutered;
...
FROM
  owners o FULL
  OUTER JOIN animals a ON o.id = a.owner_id;
...
FROM
  visits vs
  INNER JOIN animals a ON a.id = vs.animal_id
  INNER JOIN vets v ON v.id = vs.vets_id
WHERE
  v.name = 'William Tatcher'
ORDER BY
  vs.date_of_visit DESC
LIMIT
  1;
...
FROM
  visits vs
  LEFT JOIN vets v ON vs.vets_id = v.id
WHERE
...
FROM
  specializations sp FULL
  OUTER JOIN species s ON s.id = sp.species_id FULL
  OUTER JOIN vets v ON v.id = sp.vets_id;
...
WHERE
  ...
  AND vs.date_of_visit BETWEEN 'Apr 1, 2020'
  AND 'Aug 30, 2020';
...
FROM
  visits vs
  LEFT JOIN vets v ON v.id = vs.vets_id
  LEFT JOIN specializations sp ON sp.vets_id = vs.vets_id
WHERE
  sp.id IS NULL
GROUP BY
  v.name;
